...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        s3_images = [obj.key for obj in bucket.objects.filter(Prefix='product_images/') if obj.key != 'product_images/']
        ...
        orphaned_images = [image for image in s3_images if image not in db_images and image not in initial_db_images]
