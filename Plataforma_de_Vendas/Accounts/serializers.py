...
class CustomUserSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if self.instance:
            if self.instance.username != data.get('username') and CustomUser.objects.filter(username=data.get('username')).exists():
                errors['username'] = 'Username already exists'
            if self.instance.email != data.get('email') and CustomUser.objectsfilter(email=data.get('email')).exists():
    ...
    def create(self, validated_data):
        validated_data['phone_number'] = self.format_phone_number(validated_data.get('country_phone_number_code'), validated_data.get('phone_number'))
...
class ExistingUserSerializer(CustomUserSerializer):
    ...
    def validate(self, data):
        ...
        if self.instance:
            if self.instance.username != data.get('username') and CustomUser.objects.filter(username=data.get('username')).exists():
                errors['username'] = 'Username already exists'
            if self.instance.email != data.get('email') and CustomUser.objects.filter(email=data.get('email')).exists():
                errors["email"] = "Email already exists"
